...
**Update after migrating the maples and alder (all six species rendered side by side under `-d3d11`, Nanite)**: EVERY species (birch, red maple, sugar maple A/B, alder A/B) renders bare in our engine, not just the birch. The maple kits DO ship a leaf "Cap" material with an opacity texture, but no leaves appear; all materials report opaque blend mode and single-sided, every mesh has Nanite enabled with fallback relative error 1.0, and LOD0 has 2277-2768 vertices. Likely cause (UNCONFIRMED): the Nanite fallback used when Nanite is unsupported drops the thin leaf cards. Not investigated further; leafy trees, and therefore seasons based on foliage, are not currently possible.

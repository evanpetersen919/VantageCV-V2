...
### [RESOLVED] Horizon "ribbon" artifact -- WorldPartitionHLOD proxies, a class the terrain-hiding logic never matched
The pale, wavy strip at the far horizon (see the "Still a real artifact" note below, from the environment/ground entry) was investigated, not left unknown. It survived hiding Landscape actors, the Landscape show flag and a much denser fog, so a live `DebugListActorsWithMesh` query (new permanent debug RPC, alongside the new `DebugConsoleCommand` used to test hypotheses like `showflag.staticmeshes 0` without a rebuild) was used to inspect exactly what static mesh actors exist and their real hidden state, instead of guessing at more hooks. Two real, distinct findings, both confirmed live:

1. `showflag.staticmeshes 0` removed the ribbon entirely while the sky/clouds stayed, proving it was a specific static mesh, not terrain or atmosphere. The engine's own `/Engine/EngineSky/SM_SkySphere` (a `StaticMeshActor` with a painted mountain/cloud dome texture) exists in the template map -- but querying it directly showed `actor_hidden: true` (our own hide logic already caught it) while the ribbon was still visible, ruling it out as the cause despite looking like a strong match by name/texture alone.
2. The real cause: 64 `WorldPartitionHLOD` actors -- Epic's own auto-generated merged-mesh proxies standing in for distant/unloaded Landscape tiles -- a completely different class from `Landscape`/`LandscapeStreamingProxy` or `StaticMeshActor`, so the existing class-name check never matched them. `ProceduralScenarioLoader::HideIfTemplateTerrain` (refactored out of the duplicated inline predicate, now shared by the initial actor scan and reused where a streaming-timing fix was tried and ruled out -- see below) now also matches `ClassName.Contains("HLOD")`. Verified live: all 64 report `actor_hidden: true` after the fix, and the ribbon is gone in both a bare-ground scene and a real generated city, from multiple camera angles including ones that stress different World Partition cells.

**A real dead end, kept for the next person**: before finding the actual cause, a `FOnActorSpawned` delegate and then a `FWorldDelegates::LevelAddedToWorld` hook were added on the theory that World Partition streams the template's actors in late, after the one-time actor scan. Both were real, correctly-implemented fixes for a real potential problem (confirmed: only 46 of ~138 template actors were hidden by the initial scan alone) -- but neither changed the ribbon, because the actual culprit (`WorldPartitionHLOD`) was `is_spatially_loaded: false` (always resident, not streamed late) and was already being caught by the initial scan once its class was added to the match list. The `LevelAddedToWorld` hook is still real and still kept (it correctly catches anything that IS streamed in late), just not what fixed this specific bug.

### [VERIFIED] Full-size (500m) scenario load: 299 buildings, ~68,000 assets, no crash
Every prior live verification in this project used a 200m area (roughly 10-11k assets); the real `urban_dense` template generates a 500m area. Generated and sent a real full-size scenario (seed 42, bounds -250..250m): 299 buildings, 61,090 building facade pieces, 3,200 road-edge pieces, 2,400 street-furniture pieces, 67,809 total assets, 16.9MB of JSON. `generate_scenario`+`serialize_scenario` together took under 1 second. The UE5 load (`LoadProceduralScenario` RPC, includes building/spawning all actors) took 28.7s. No crash; engine memory settled around 5.9GB. Verified live from directly above, at an oblique angle and at street level -- a genuine multi-block city with varied buildings, real roofs/AC units, streets, lamps and vehicles receding into the distance. One GPU-hung crash occurred on an editor launch attempt before the scenario was even sent (the familiar first-launch shader-compile flakiness documented elsewhere in this file); a relaunch was unaffected and loaded cleanly.

### [RESOLVED, no foliage] Scenario seasons: winter, spring, summer, fall
`environment.py` gained `Season` and per-season environment presets (`season_environment`), and `generate_scenario(..., season=None)` records `ScenarioResult.season` (fixed by the caller, or drawn uniformly from the scenario seed as the LAST draw of the style stream so every earlier style choice for a seed is unchanged). A season sets the sun angle and colour temperature, fog, exposure, saturation and an optional colour gain (`EnvironmentConfig` gained `sun_temperature_k` and `color_gain`, both already understood by the plugin, so no C++ change) and decides whether street trees appear: only winter and fall keep them, because Epic's tree kits are bare branch skeletons (see the tree entries below), so leafy seasons simply have no street trees rather than dead-looking ones. `bin/send_scenario_to_ue5.py` sends the scenario's own season preset. The preset values are tuned by eye against this scene, NOT measured: summer high bright sun (-58 deg pitch), spring softer hazier light (-45, 6800K), fall low warm sun (-24, 4600K, warm gain), winter very low cool sun (-16, 8500K, cool gain, saturation 0.6, heavier fog). Verified live by rendering one real city in each season (summer bright and tree-free, fall warm gold with bare trees, winter cool and muted with bare trees). Not done: snow or wet ground, leaf colour, seasonal sky/cloud changes, a time-of-day axis.

### [RESOLVED] Roofs now use Epic's own roof materials and carry rooftop equipment
Follow-up to the roof-slab entry below. `roofs.py` now gives each building one of four migrated Epic roof materials (`MI_Rooftop_BitumenRoofing`, `DirtyConcreteTiles`, `PebbleDash`, `RoofTile`, tags `roof_0`..`roof_3`, chosen by `building_id % 4`; Epic picks one of six by `Building_ID % 6`, two are not migrated) instead of the borrowed asphalt, and `generate_roof_prop_pieces` scatters real rooftop equipment on each slab (`Kit_roof_AC_Unit_B`, `Kit_Roof_Vent_RR`, `Kit_roof_exhaust_RR`, sizes measured in the engine). The prop LAYOUT is INFERRED, not measured (Epic places whole prop Blueprints from a 1.2GB kit whose internals were not read): about one item per 60m2 of roof (max six), 3m apart, 1.5m from the edge, random 90-degree yaw, deterministic per building. Verified live in a real generated city from above: varied grey/tan roofs with visible AC units and vents. Rooftop props are only emitted when the scene is dressed (with an environment), like the ground and roofs.
...
**Update after migrating the maples and alder (all six species rendered side by side under `-d3d11`, Nanite)**: EVERY species (birch, red maple, sugar maple A/B, alder A/B) renders bare in our engine, not just the birch.

**Investigated and concluded (not a rendering bug)**: the earlier suspicion that the D3D11 Nanite fallback drops leaf cards was tested and is only partly right. Turning Nanite off on tree meshes at spawn (`VehicleActorSpawner.cpp`, `#if WITH_EDITOR`, meshes under `/Kit_Tree_`, which rebuilds them from full source geometry) does restore fine twigs (alder B especially) and is kept, but still shows NO leaves. Reading the materials headlessly in CitySample shows why: the leaf materials (`Cap_Mat`, `Maple_Bough_Mat`) are opaque, single-sided and have only a base-colour texture connected (no opacity or opacity-mask input, so the `Cap_Opacity` texture is unused), the alder kit has only a bark material, and City Sample has no separate leaf/foliage assets. Epic's tree kits ship as bare branch skeletons; foliage is not part of these assets. Leafy trees, and foliage-based seasons, would need a different tree asset source or hand-built canopies.
...
**Update**: the horizon "ribbon" was investigated and fixed -- see the `[RESOLVED] Horizon "ribbon" artifact` entry above this one (root cause: `WorldPartitionHLOD` proxy actors, a class the terrain-hiding logic didn't match). The road-gloss and open-topped-building notes below were already separately fixed elsewhere in this file (real matte asphalt material; roof slabs).

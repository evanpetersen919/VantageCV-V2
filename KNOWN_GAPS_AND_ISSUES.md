...
`FACADE_WALL_MODULE_METERS`/`FACADE_CORNER_TO_FIRST_WALL_METERS` in `building_placement.py` and the tiling loop in `building_facade.py` now use these real values.

**Remaining defect, itself now also grounded in real evidence, not just observed**: a corner only guarantees a flush connection to the edge that starts at it (its own rotation) -- not to the perpendicular edge that also touches it. This isn't unique to our implementation: the real point cloud shows Epic's own actual generated buildings have the exact same non-zero "remainder" gap at that far corner connection (e.g. one real building's real far-corner gap measured 541.8cm, clearly not zero) -- their system tiles each edge independently from its own corner and accepts whatever's left, same as ours. The difference is how visible it reads: our current corner rotation/offset choice (matching each corner's own edge, `rotation_rad = edge_index * 90`) produces a more visually pronounced gap at the far corner than Epic's real buildings typically show, and this has NOT been fully closed. Live-checked from a realistic street-level angle (not just top-down): the result currently reads as a building with deep-set corner piers and recessed glazing rather than a cleanly closed box -- not obviously broken, but not fully flush either.
...
**Concrete next step**: City Sample ships `SM_BLDG_CHA_L01_A_CornerExL_01_N1`/`...CornerExR_01_N1` (mirrored L/R variants of the plain corner) -- not yet wired in or verified. The same point-cloud technique used to nail module spacing this session could also, with more time, be pointed at a real building whose corners use these L/R variants (if any do) to get their real rotation/offset directly, the same rigorous way, rather than guessing.

See `src/procedural/building_facade.py`'s own module docstring for the full technical derivation and the precise falsifiable tests that found the two fixed bugs.

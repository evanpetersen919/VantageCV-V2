...
**Still open, deliberately not attempted here**: vehicle paint materials/textures. City Sample's real vehicle paint material system (`M_Veh_CarPaint` and every `MI_Veh_*` built on it) chains through material functions that depend on `/Traffic/MaterialFunctions/MF_UnpackTrafficVehicleInstanceCustomData`, living in Epic's separate MassTraffic plugin (a full Mass/ZoneGraph AI traffic system, not portable content) -- UE5 automatically falls back to its own default flat material for every affected slot. Real research (via a dedicated background research pass) found no publicly documented fix; one unverified lead worth testing (a "Paint Variation" toggle reportedly exposing a plain `BaseColor` parameter on the material instances, bypassing the broken procedural system) has not yet been tried. Brake pads and the visible engine were deliberately not added (mostly hidden behind wheel rims / under a closed hood -- low visual payoff for the added per-vehicle catalog complexity); `vehCar_vehicle13`'s unique turn-signal meshes were added since they were already being touched, `SM_Wheel_MotionBlur*`/`SM_MotionBlur_*` meshes were deliberately excluded (motion-blur effect geometry, meaningless for a frozen-frame static scene).

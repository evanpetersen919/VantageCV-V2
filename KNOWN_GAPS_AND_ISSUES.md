...
### [RISK] Buildings were flat textured boxes -- now real modular City Sample facades (walls/corners, tiled), but corner-to-perpendicular-wall joins are not flush
**Reopened**: this entry was previously marked RESOLVED after full-scenario live verification looked clean from the angles checked at the time. A later, more rigorous session (heavily debugging a user-reported "walls not lined up, corners in the wrong spot" regression) found the earlier verification had not actually caught a real defect: buildings do NOT close into complete, gap-free rectangles from directly overhead. Two real, separate, confirmed bugs (described below, in `generate_building_facade_pieces`/`_rotate_2d`) were found and fixed this session, and are genuinely solid now (verified via exact numeric bounding-box reconstruction across all 4 corner rotations, not just rotation 0, plus live screenshots). One further real, root-caused issue remains and is NOT fixed:

**Remaining defect, proven not just observed**: a corner piece's connection to the wall run of the OTHER (perpendicular/incoming) edge it touches is not flush -- there is a visible gap or overlap there. This was proven mathematically, not merely seen in a screenshot: the `CornerEx` asset's real measured local bounds (via the live `GetStaticMeshBounds`/`GetStaticMeshFootprintAtHeight` debug RPCs) are asymmetric in a way that makes it geometrically impossible for any single rigid (translation-only) pivot offset to simultaneously flush-match BOTH adjacent walls -- an exact least-squares fit of the flush-match equations has no zero-residual solution. **Real, very likely fix, found but not yet shipped**: a real content search of the mounted City Sample assets found City Sample ships `SM_BLDG_CHA_L01_A_CornerExL_01_N1` and `...CornerExR_01_N1` -- mirrored L/R variants of this exact corner mesh, alongside `CornerIn`/`CornerInL`/`CornerInR` (interior-corner counterparts), clearly intended for exactly this situation (alternating handedness per corner so each corner's two arms are individually correct for both edges it touches). A quick numeric fit found a promising perpendicular-alignment offset for `CornerExL`, but its own "own-edge" offset was not independently re-derived/confirmed live, so wiring L/R selection into `generate_building_facade_pieces` was deliberately deferred rather than shipped half-verified. **Concrete next step**: measure `CornerExL`'s own-edge offset the same way `CornerExR`'s (== plain `CornerEx`'s) was derived (`(0.0, 1.5)` in `building_placement.py`), determine the correct L/R alternation pattern around a rectangle's 4 corners (likely diagonal: two opposite corners use L, the other two use R, or similar), and verify live before shipping.

**Two real, separate, now-fixed bugs found this session while chasing the above**:
...
2. `_rotate_2d` (used for every position offset in `building_facade.py`) was rotating by `-rotation_rad` instead of `+rotation_rad` -- conflating "how the mesh visually rotates in UE5" (genuinely `-rotation_rad`, via `ProceduralScenarioLoader.cpp`'s Yaw conversion) with "how a position offset for placing that mesh should be computed" (a different quantity). Found via a precise, falsifiable live test: a corner+wall pair confirmed flush at rotation 0 was rigidly rotated to rotation `-pi/2`; the old (negated) position formula broke the flush join, the corrected (non-negated) one preserved it exactly. This also flips the corner-index-to-rotation formula from `-edge_index * 90°` to `+edge_index * 90°` (re-derived and verified numerically against all 4 real edge directions). A new test, `test_corner_piece_rotation_matches_its_own_edge_direction` in `tests/unit/test_building_facade.py`, asserts the absolute rotation-to-edge-direction mapping -- the older spacing/count tests could not have caught this sign bug, since consecutive-wall spacing stays correct regardless of which absolute direction "forward" is.

See `src/procedural/building_facade.py`'s own module docstring for the full technical derivation of both fixes and the precise falsifiable tests that found them.

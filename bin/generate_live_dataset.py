...
async def _run(args: argparse.Namespace) -> None:
    ...
    print(
        f"{len(result.frames)} frames in {result.elapsed_seconds:.0f} s -> {output_dir} "
        f"(calibration {result.calibration_error_px:.2f} px, skipped scenarios "
        f"{result.skipped_scenarios})"
    )
